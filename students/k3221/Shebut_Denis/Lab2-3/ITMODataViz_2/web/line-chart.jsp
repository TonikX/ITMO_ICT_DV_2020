...
<div class="content-wrapper">
    ...
    <header class="header">
        <p style="font-size: 30px; margin: 0; padding: 10px 1em;">Лабораторная работа №2.<br>
            Оценка периодичности в энергопотреблении за 5 лет</p>
    </header>
                ...
                <script type="text/javascript">
                    ...
                    d3.csv("data/total_data.csv")
                        ...
                        .get(function (error, rows) {
                            ...
                            const svg = d3.select("main").append("div").attr("class", "svg-container").append("svg").attr("height", "100%").attr("width", width + margin.left + margin.right);
...
<div class="containter">
    ...
    <ul>
        <li>Оценка периодичности в энергопотреблении за 5 лет</li>
        <li><a href="mean-daily-line-chart.jsp">Оценка периодичности в энергопотреблении за 5 лет (среднее ежечасное енергопотребление по дням)</li>
        <li><a href="bar-chart.jsp">Среднее ежечасное энергопотребление в течение дня/недели</a></li>
    ...
    <script type="text/javascript">
        ...
        d3.csv("data/total_data.csv")
            ...
            .get(function (error, rows) {
                ...
                const yAxis = d3.svg.axis().orient("left").scale(y);
                const xAxis = d3.svg.axis().scale(x).orient("bottom").tickFormat(d3.time.format("%Y-%m-%d %H:%M:%S"));
